...
const Layout = ({onLogout, user}) => {
    ...
    const fetchTasks = useCallback (async () => {
        ...
        try {
            ...
            const {data} = await axios.get(" http://localhost:7000/api/tasks/tm", {
            ...
            console.error(error);
            setError(error.message || "Couldn't load tasks.")
    ...
    const StartCard = ({title, value, icon}) => {   
        ...
        duration-300 hover:border-blue-100 group">
            <div className="flex items-center gap-2">
                <div className="p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-blue-500/10 group-hover:from-fuchsia-500/20
                group-hover:to-blue-500/20">
                    {icon}
                </div>
                <div className="min-w-0">
                    <p className="text-lg sm:text-xl font-bold bg-gradient-to-r from-fuchsia-500 to-blue-600 bg-clip-text text-transparent">
                        {value}
                    </p>
    ...
    //LOAD
    if (loading) return (
        <div className="min-h-screen bg-gray-50 items-center justify-center flex">
    ...
    if (error) return (
        <div className="min-h-screen bg-gray-50 items-center justify-center flex">
            <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 max-w-md">
                ...
                <button className="mt-4 py-2 px-4 bg-red-100 text-red-700 rounded-lg text-sm font-medium hover:bg-red-200
                transition-colors">
                    Try Again
                </button>
            </div>
                    ...
                    <div className="xl:col-span-1 space-y-4 sm:space-y-6">
                        <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-blue-100">
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                                <StartCard title='Total task' value={stats.totalCount} icon={<Circle className="w-3.5 h-3.5 sm:w-4
                                sm:h-4 text-blue-500"/>}/>
                                <StartCard title='Completed' value={stats.completedTasks} icon={<Circle className="w-3.5 h-3.5 sm:w-4
                                sm:h-4 text-green-500"/>}/>
                                <StartCard title='Pending' value={stats.pendingCount} icon={<Circle className="w-3.5 h-3.5 sm:w-4
                                sm:h-4 text-fucshia-500"/>}/>
                                <StartCard title='Completion Rate' value={`${stats.completedPercentage}%`} icon={<Zap className="w-3.5 h-3.5 sm:w-4
                                sm:h-4 text-blue-500"/>}/>
                            </div>
                                ...
                                <div className="relative pt-1">
                                    <div className="flex gap-1.5 items-center">
                                        <div className="flex-1 h-2 sm:h-3 bg-blue-100 rounded-full overflow-hidden">
                                            <div className="h-full bg-gradient-to-r from-fuchsia-500 to-blue-600 transition-all
                                            duration-500" 
                        ...
                        <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-blue-100 ">
                            ...
                            <div className="space-y-2 sm:space-y-3">
                                {tasks.slice(0,3).map((task) => (
                                    <div key={task._id || task.id} className="flex items-center justify-between p-2 sm:p-3 hover:bg-blue-50/50 rounded-lg
                                    transition-colors duration-200 border border-transparent hover:order-blue-100">
                                ...
                                <div className="text-center py-4 sm:py-6 px-2">
                                    <div className="w-12 h-12 sm:w-16 mx-auto sm:mb-4 rounded-full
                                    bg-blue-100 flex items-center justify-center">
                                        <Clock className="w-6 h-6 sm:h-8 text-blue-500"/>
                                    </div>
