...
const TaskItem = ({task, onRefresh, showCompleteCheckbox=true, onLogout}) => {
    ...
    const handleComplete = async () => {
        ...
        try {
            await axios.put(`${API_URL}/${task.id}/tm`, {completed: newStatus},
    ...
    const handleDelete = async (params) => {
        try {
            await axios.delete(`${API_URL}/${task._id}/tm`, {headers: getAuthHeaders()})
            onRefresh?.()
        } catch (error) {
            if (error.response?.status === 401) onLogout?.()
        }
    ...
    const handleSave = async (updatedTask) => {
        try {
            const payload = (({title, description, priority, dueDate, owner, completed}) => 
                ({title, description, priority, dueDate, owner, completed}))(updatedTask)
            await axios.put(`${API_URL}/${task._id}/tm`, payload,
                {headers: getAuthHeaders()})
                    ...
                    {showMenu && (
                        <div className={TI_CLASSES.menuDropdown}>
                            {MENU_OPTIONS.map(option => (
                                <button key={option.action} onClick={() => handleAction(option.action)}
                                className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-blue-50
